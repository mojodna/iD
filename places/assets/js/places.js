...
function backToParkTiles() {
  var location = window.location,
    split = location.hash.replace('#', '').split('&')[1].replace('map=', '').split('/');

  mode = 'view';
  window.location = location.origin + location.pathname + '#' + split[0] + '/' + split[2] + '/' + split[1];
  window.location.reload();
  document.getElementById('back-to-park-tiles').style.display = 'none';
...
function startEditing() {
  ...
  mode = 'edit';
  window.location = location.origin + location.pathname + '#background=Esri&map=' + split[0] + '/' + split[2] + '/' + split[1] + '&mode=edit';
  window.location.reload();
  document.getElementById('back-to-park-tiles').style.display = 'block';
...
window.onload = function() {
  ...
  if (hash) {
    if (hash.indexOf('&mode=edit') !== -1) {
      mode = 'edit';
      src = '../dist/index.html' + hash;
      title = 'Places';
    }
  ...
  document.getElementById('back-to-park-tiles').style.display = (title === 'Park Tiles' ? 'none': 'block');
  document.getElementById('start-editing').style.display = (title === 'Park Tiles' ? 'block': 'none');
